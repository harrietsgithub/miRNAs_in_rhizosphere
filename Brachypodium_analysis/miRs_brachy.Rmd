...
The aim of this experiment is to analyse the miRNA composition of the rhizosphere of Brachypodium distachyon. Only 2/3 rhizosphere samples passed the sequencing process and 3/3 unplanted soil samples.
...
biom_file<-import_biom("C:/Users/hmiddleton/Dropbox/Projet_Mutants_miPEPs_miRNA/Harriet/miRs_rhizo_bact_exosomes/miRs_brachy/mirna_bra_brut_95.biom1")

sample<- data.frame(fread("C:/Users/hmiddleton/Dropbox/Projet_Mutants_miPEPs_miRNA/Harriet/miRs_rhizo_bact_exosomes/miRs_brachy/mapping_file_95.txt", sep="\t"), check.names=F)
...
```{r}
phylo_filtered = filter_taxa(phylo, function(x) sum(x >= 1)>= (1), TRUE)
phylo_filtered 
...
vplot$delabel <- do.call(rbind, strsplit(tmp, "i-", fixed=TRUE))[,2]
...
write.table(sigtab, 'sigtab_bra_DESEQ.txt')
```
...
rhizo <- c("HI.4536.002.NEBNext_Index_4.B1","HI.4536.002.NEBNext_Index_6.B3")
...
top_normalized$cluster <- do.call(rbind, strsplit(c_split, "i-", fixed=TRUE))[,2]
